...
function usage() {
    ...
    echo
    echo "Example:"
    echo " $0 deploy --project-suffix demo"
    echo
    echo "COMMANDS:"
...
LOGGED_IN_USER=$(oc ${ARG_OC_OPS} whoami)
OPENSHIFT_USER=${ARG_USERNAME:-$LOGGED_IN_USER}
PRJ_SUFFIX=${ARG_PROJECT_SUFFIX:-`echo $OPENSHIFT_USER | sed -e 's/[-@].*//g'`}

GITHUB_ACCOUNT=${GITHUB_ACCOUNT:-OpenShiftDemos}
GITHUB_REF=${GITHUB_REF:-ocp-3.7}

# https://github.com/OpenShiftDemos/openshift-cd-demo/blob/ocp-3.7/cicd-template.yaml
# https://raw.githubusercontent.com/OpenShiftDemos/openshift-cd-demo/ocp-3.7/cicd-template.yaml
GITHUB_PATH=https://raw.githubusercontent.com/${GITHUB_ACCOUNT}/openshift-cd-demo/${GITHUB_REF}
...
function deploy() {
  ...
  if [ $LOGGED_IN_USER == 'system:admin' ] ; then
  ...
  local template=$GITHUB_PATH/cicd-template.yaml
  echo "Using template $template"
  oc $ARG_OC_OPS new-app -f $template \
    --param DEV_PROJECT=dev-$PRJ_SUFFIX \
    --param STAGE_PROJECT=stage-$PRJ_SUFFIX \
    --param=WITH_SONAR=$ARG_DEPLOY_SONAR \
    --param=WITH_CHE=$ARG_DEPLOY_CHE \
    --param=EPHEMERAL=$ARG_EPHEMERAL \
    -n cicd-$PRJ_SUFFIX
}
...
if [ "$LOGGED_IN_USER" == 'system:admin' ] && [ -z "$ARG_USERNAME" ] ; then
